function outcome = smokChurnSimulateEvent(prevalence, scalingFactor, scalingExpon, default, ageBracTable, age, sex)
...
%   scalingFactor   scales prevalence number
%   scalingExpon    exponent on the scaling factor, can be vector by ind.
...
if size(prevalence,2) == 1
    for ii = 1:numPpl
        outcome(ii) = fate(ii) < (prevLookup(age(ii)+1,1)*(scalingFactor^scalingExpon(ii)));
    end
else
    for ii = 1:numPpl
        outcome(ii) = fate(ii) < (prevLookup(age(ii)+1,sex(ii))*(scalingFactor^scalingExpon(ii)));
    end
end
% Done!
